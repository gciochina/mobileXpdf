...
#define WM_KBD_MOUSEWHEEL	L"WM_KBD_MOUSEWHEEL"
...
, m_FindPage(NULL, gTrue, 0., gFalse, gFalse)
...
CChildView::~CChildView()
{
	...
	m_PasswordDialog.DestroyWindow();
	m_AcknowledgeDialog.DestroyWindow();
}
...
{
    if ( pMsg->message == WM_USER_GOTOPAGE )
    {
        OnMessageGoToPage( pMsg->wParam, 0 );
        return TRUE;
    }
...
bool CChildView::loadFile(CString fileName)
{
	...
	int unicodeLength = fileName.GetLength();
	_tcsncpy_s( unicodePath, MAX_PATH, fileName.GetString(), unicodeLength );
	...
	m_PDFDoc = new PDFDoc(unicodePath,unicodeLength,NULL,NULL);
	...
	while ( !m_PDFDoc->isOk() ) 
	{
		...
		{
			CPasswordDialog &pwd = m_PasswordDialog;

			if (pwd.DoModal() == IDCANCEL)
			{
				delete m_PDFDoc;
				m_PDFDoc=NULL;
				return false; // abandon load
			}
			...
			m_PDFDoc = new PDFDoc(unicodePath,unicodeLength,&gpw,&gpw);
		} 
        else 
		{
			delete m_PDFDoc;
...
#ifdef MANAGE_BOOKMARKS_ON_OPEN
...
void CChildView::OnMenuOpen()
{
	...
	if (OpenDialog.strOpenedPath.Find(_T("Cancel")) > -1)
		return;
	if (loadFile(OpenDialog.strOpenedPath))
	{
		m_renderDPI = ZoomLevel();
		m_renderDPI = m_renderDPI/ 10;
		renderPage();
	}
	else {
		MessageBox(L"Failed to load file.",L"Error");
		//dynamic_cast<CMainFrame*>(GetTopLevelFrame())->setCaption(L"MobileXpdf");
		((CMainFrame*)AfxGetMainWnd())->setCaption(L"MobileXpdf");//safer method
...
// 	CFileDialog fileDialog(TRUE, L"pdf", L"*.pdf", OFN_FILEMUSTEXIST, L"PDF Documents (*.pdf)|*.pdf|All Files (*.*)|*.*||", this);
// 	if (fileDialog.DoModal()==IDOK){
// 		CString filename = fileDialog.GetPathName();


// 	LPOPENFILENAMEW xFile;
// 	GetOpenFileName(xFile);
//	CString filename = (CString)"\\Storage Card\\test.pdf";
//	CString filename = (CString)xFile->lpstrFile;
// 	if (loadFile(filename))
// 	{
// 		renderPage();
// 	}
	...
	if (m_PDFDoc != NULL) {
		if (m_IsInDragZoomMode) {
			...
			if (factor>0.0){
				m_renderDPI*= factor;
				renderPage();
				GotoView((int)((m_ViewOffsetX+m_LastDragRect.left)*factor),(int)((m_ViewOffsetY+m_LastDragRect.top)*factor));
...
{
	m_renderDPI = m_renderDPI * ZOOM_FACTOR;
	RefreshPageView( (int)(m_ViewOffsetX * ZOOM_FACTOR), (int)(m_ViewOffsetY * ZOOM_FACTOR) );
    SetFocus();
}
...
DWORD WINAPI CChildView::RenderingThread (LPVOID param)
{
	...
	{
				cv->MessageBox(L"Out of memory. Quitting.");
	...
	if (m_PDFDoc != NULL) {
		...
		if (m_TextOutDialog.GetSafeHwnd()!=NULL && m_TextOutDialog.IsWindowVisible()) {
			...
			if (testAllocation==NULL)
			{
				...
				m_WaitDialog.setWaitMessage(L"Cannot zoom in that close. Resetting zoom.");
				m_WaitDialog.ShowWindow(SW_SHOWNA);
				m_WaitDialog.RedrawWindow(0,0,RDW_INVALIDATE | RDW_UPDATENOW | RDW_ERASE);
				
				// zoom out until page can be rendered
				int stepout = 0;
				while (testAllocation==NULL && stepout < 20)
				{
					m_renderDPI = m_renderDPI * ZOOM_FACTOR; // single zoom out
					newbytes = (m_renderDPI/72.0)*(m_renderDPI/72.0) * m_PDFDoc->getPageCropWidth(m_CurrentPage)* m_PDFDoc->getPageCropHeight(m_CurrentPage); //new approx. number of pixels
					newbytes = newbytes * 3 + newbytes*2; //24-bit splashbitmap, 16-bit gdi-bitmap
					newbytes *= 3; //Safety area;
					newbytes -= m_splashOut->getBitmap()->getWidth()*m_splashOut->getBitmap()->getHeight()*3; //substract old 24-bit splash bitmap size
					if (newbytes < 0) newbytes = 1; //do not allocate memory with negative number of bytes
					testAllocation = malloc((int)newbytes);
					stepout++;
				}
			} 
			if (testAllocation!=NULL)
			...
			m_Bitmap->SetBitmapDimension(m_splashOut->getBitmap()->getWidth(),m_splashOut->getBitmap()->getHeight());
				...
				m_WaitDialog.ShowWindow(SW_HIDE);
				m_AcknowledgeDialog.setAcknowledgeMessage(L"Out of memory. Quitting.");
				if ( m_AcknowledgeDialog.DoModal() == IDCANCEL ) 
				{
			...
			//prerender next page
			/*if (m_renderingThread!=0){
				m_WaitDialog.setWaitMessage(L"Warning: Old thread still existing!");
				m_WaitDialog.ShowWindow(SW_SHOWNA);
				m_WaitDialog.RedrawWindow(0,0,RDW_INVALIDATE | RDW_UPDATENOW | RDW_ERASE);
				DWORD exitcode=0;
						GetExitCodeThread(m_renderingThread->m_hThread,&exitcode);
						if (exitcode==STILL_ACTIVE){
							m_WaitDialog.setWaitMessage(L"Warning: Old thread still running!");
							m_WaitDialog.ShowWindow(SW_SHOWNA);
							m_WaitDialog.RedrawWindow(0,0,RDW_INVALIDATE | RDW_UPDATENOW | RDW_ERASE);
						}
			}
		...
		wchar_t caption[128];
		wsprintf(caption,L"/%d",m_PDFDoc->getNumPages());
#ifndef WIN32_PLATFORM_WFSP
		//dynamic_cast<CMainFrame*>(GetTopLevelFrame())->setCaption(caption);

		((CMainFrame*)AfxGetMainWnd())->setCaption(caption);//safer method
...
/*
	...
	{
		wchar_t zoomLevel[256];
		DWORD size=512;
		DWORD type=REG_SZ;
		RegQueryValueEx(key,0,0,&type,(BYTE*)zoomLevel,&size);
		RegCloseKey(key);
		
		res  = (zoomLevel[0] - 48) * 100;
		res += (zoomLevel[1] - 48) * 10;
		res += zoomLevel[2] - 48;
	}

	if (res < 100 || res > 500 )
		return 100;	//safety first!	
	else
		return res;
*/
}
...
{
 //   m_GotoPageDialog.mCurrentPage = m_CurrentPage;
 //   m_GotoPageDialog.mNumOfPages = m_PDFDoc->getNumPages();
 //   m_GotoPageDialog.mPageNumberStr = _T("");

	//if ( m_GotoPageDialog.DoModal() == IDOK ) 
 //   {
 //       m_ViewOffsetX = 0; 
 //       m_ViewOffsetY = 0;
 //       
 //       int pageNum = _ttoi( m_GotoPageDialog.mPageNumberStr );
 //       if ( pageNum != 0 )
 //           GotoPage( pageNum, m_ViewOffsetX, m_ViewOffsetY);
 //   }
...
void CChildView::execFunction(CConfigDialog::Functions function)
{
	...
	RECT clientArea;
	GetClientRect(&clientArea);
	int deltaY = 1;//(clientArea.bottom-clientArea.top)/VERT_SCROLL_STEPS_PER_PAGE;
	int deltaX = 1;//(clientArea.right-clientArea.left)/HORZ_SCROLL_STEPS_PER_PAGE;
...
LRESULT CChildView::PostCreateWindow(WPARAM wParam, LPARAM lParam)
{
	...
	if (info.m_strFileName.Compare(L"")!=0){
		...
		{
			renderPage();
			AfxGetMainWnd()->PostMessage( WM_USER_REFRESHTOOLBAR ); // refresh state of toolbar buttons
		}
		else
		{
			if ( m_AcknowledgeDialog.DoModal() == IDCANCEL ) 
...
void CChildView::GotoPage(int pageno, int x, int y, BOOL redraw)
{
		...
		&& pageno <= m_PDFDoc->getNumPages())
	{
		...
		AfxGetMainWnd()->PostMessage( WM_USER_REFRESHTOOLBAR );
...
void CChildView::ScrollScreen(int dir)
{
	...
	case LEFT_SCREEN:
		if (m_ViewOffsetX > 0)
			...
			switch (m_Rotation){
				case 90:
					AfxGetMainWnd()->PostMessage( WM_COMMAND, ID_NEXT_PAGE );
					break;
				case 270:
					AfxGetMainWnd()->PostMessage( WM_COMMAND, ID_PREV_PAGE );
					break;
				default:
...
BOOL CChildView::FindText(int searchPage, BOOL reverse)
{
	...
	TextOutputDev FindPage(NULL, gTrue, 0., gFalse, gFalse);
	...
	while(true) {
		...
		rc = FindPage.findText(ucstring, length,
			...
			m_FindDlg.m_caseSensetive, backward, false,
			&x0, &y0, &x1, &y1);
...
LRESULT CChildView::OnMessageGoToPage( WPARAM wParam, LPARAM lParam ) 
{
    m_ViewOffsetX = 0; 
    m_ViewOffsetY = 0;
