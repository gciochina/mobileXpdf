...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	...
	ON_MESSAGE(WM_USER_REFRESHTOOLBAR,&CMainFrame::RefreshToolbar)
#ifdef MOUSE_PLUG_HANDLING
    ON_MESSAGE(WM_MOUSEPLUGGED, &CMainFrame::OnMousePlugged ) 
    ON_MESSAGE(WM_MOUSEUNPLUGGED, &CMainFrame::OnMouseUnplugged ) 
#endif
...
{
#ifdef MOUSE_PLUG_HANDLING
    cMouseDetection::GetInstance().Finalize();
#endif
	m_wndView.DestroyWindow();
	m_wndCommandBar.DestroyWindow();
	m_menuToolbar.DestroyWindow();
}
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	// PATCH: old command bar will still be shown in order to recalculate child view area properly	
	...
	m_wndCommandBar.SetBorders( 0, 29, 0, 0 );
	m_wndCommandBar.SetBarStyle(m_wndCommandBar.GetBarStyle() | CBRS_SIZE_FIXED);
	// end of PATCH

	int menutoolbar_id = IDD_MENUTOOLBAR;

	RECT rect;
	m_wndCommandBar.GetWindowRect(&rect);
	if ( rect.right - rect.left < 1024 )
		menutoolbar_id = IDD_MENUTOOLBARSMALL;

	if ( !m_menuToolbar.Create( menutoolbar_id, this ) )
	{
	...
	m_menuToolbar.GetWindowRect(&rect);
	m_menuToolbar.GetDesktopWindow()->GetWindowRect(&rectDesktop);

	int y = rect.bottom - rect.top;
	int cx = rectDesktop.right - rectDesktop.left;
	int cy = (rectDesktop.bottom - rectDesktop.top) - y;

	m_menuToolbar.SetWindowPos( NULL, 0, 0, cx, y, SWP_NOZORDER );
	this->SetWindowPos( NULL, 0, y, cx, cy, SWP_NOZORDER );*/
...
#ifdef MOUSE_PLUG_HANDLING
    cMouseDetection::GetInstance().Init();
#endif
...
LRESULT CMainFrame::OnMousePlugged(WPARAM wParam, LPARAM lParam)
{
    ShowCursor( TRUE );
    return (LRESULT)0;
}

LRESULT CMainFrame::OnMouseUnplugged(WPARAM wParam, LPARAM lParam)
{
    int state = 0;
...
LRESULT CMainFrame::RefreshToolbar(WPARAM wParam, LPARAM lParam)
{
	if ( m_wndView.m_PDFDoc != NULL && m_wndView.m_PDFDoc->isOk() )
	{
        m_menuToolbar.SetPageNum( m_wndView.m_CurrentPage, m_wndView.m_PDFDoc->getNumPages() );
		if ( m_wndView.m_CurrentPage == 1 )
			m_menuToolbar.enableButtonPrev(FALSE);
		else
			m_menuToolbar.enableButtonPrev(TRUE);
		if ( m_wndView.m_CurrentPage == m_wndView.m_PDFDoc->getNumPages() )
			m_menuToolbar.enableButtonNext(FALSE);
		else
			m_menuToolbar.enableButtonNext(TRUE);
    }
	return (LRESULT)0;
}
...
// $$$AD
	...
	if (m_Fullscreen){
		...
		m_wndCommandBar.Show(TRUE); //do not hide it!!!
		RECT fsRect;
		fsRect.top=0;
		fsRect.left=0;
