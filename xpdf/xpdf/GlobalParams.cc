...
void GlobalParams::setupBaseFonts(char *dir) {
  ...
  if ( SHGetSpecialFolderPath( NULL, buf, CSIDL_FONTS, FALSE ) ) {
	char s[512];
	if ( WideCharToMultiByte(CP_ACP,0,buf,-1,s,512,0,0) != 0 ) 
    {
	    //DWORD lastError= ::GetLastError();
	    //if (lastError==ERROR_INSUFFICIENT_BUFFER)
	    //	MessageBox(NULL,L"ERROR_INSUFFICIENT_BUFFER",L"Error",MB_ICONERROR);
	    ...
	    //if (lastError==ERROR_INVALID_PARAMETER)
	    //	MessageBox(NULL,L"ERROR_INVALID_PARAMETER",L"Error",MB_ICONERROR);
      strncpy( winFontDir, s, MAX_PATH-1 );
      winFontDir[MAX_PATH] = '\0';
    }
    else
      winFontDir[0] = '\0';
  }
